...
def approx_diff(inputs, delay=2, dt=1e-1):

    """

    A more efficient method of computing velocities from position variables

    Args:
        func: Function to compute gradient of. Inputs and outputs can be
            arbitrary dimension.
        delay: By how much lookahead to samples to shift the approximate derivative?
        dt: time delay by which to compute the derivatives
    Returns:
        Gradient vector of each dimension of func with respect to each
        dimension of input.
    """
    # allocate
    inputs_delayed = np.zeros(inputs.shape)
    # copy delayed inputs into inputs_delayed array
    # inputs_delayed = np.copy(inputs)
    inputs_delayed[:-delay,:]   = np.copy(inputs[delay:,:])
    # to avoid nans in last two rows
    inputs_delayed[-delay:,:]   = np.copy(inputs[:delay,:])

    # allocate gradients spaces
    gradient = np.zeros(inputs.shape)
    # iterate through all the column vectors of given data
    for col in range(inputs.shape[-1]):

        test_input = np.copy(inputs[:,col])
        test_delayed = np.copy(inputs_delayed[:,col])
        diff = (test_input - test_delayed)

        gradient[:,col] = diff/dt
...
def finite_differences(func, inputs, func_output_shape=(), epsilon=1e-5):
    ...
    Args:
        func: Function to compute gradient of. Inputs and outputs can be
            arbitrary dimension.
        inputs: Matrix to compute gradient for.
    ...
    Returns:
        ...
        dimension of input.
    """
    print('inputs.shape: {}, func_output_shape: {}'.format(inputs.shape, func_output_shape))
    gradient = np.zeros(func_output_shape)
    # iterate through all the column vectors of given data
    for col in range(inputs.shape[-1]):
        # iterate through each vector
        for idx, _ in np.ndenumerate(inputs[:,col]):

            for i in range(len(inputs[:,col])):
                test_input = np.copy(inputs[:,col])
                # print('idx: {}, inputs col: {}'.format(idx, test_input.shape))
                test_input[i] += epsilon
                obj_d1 = func(test_input[i])
                # assert obj_d1.shape == func_output_shape
                # print('test: {}, obj_d1: {}'.format(test_input.shape, obj_d1.shape))
                test_input = np.copy(inputs[:,col])
                test_input[i] -= epsilon
                obj_d2 = func(test_input[i])
                # assert obj_d2.shape == func_output_shape
                diff = (obj_d1 - obj_d2) / (2 * epsilon)
                # print('grad: {}, diff: {}'.format(gradient.shape, diff))
                gradient[i] += diff
    return gradient
